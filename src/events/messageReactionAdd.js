    ...
    async execute(reaction, user, client) {
        ...
        if (reaction.partial) {
            try {
                ...
                logger.error('[Events: MessageReactionAdd] Failed to fetch partial reaction:', error);
                return;
            }
        }
        ...
        try {
            ...
            if (!member) {
                logger.warn(`[Events: MessageReactionAdd] Member with ID ${user.id} not found in guild.`);
                return;
            }
            ...
            if (!member.roles.cache.has(roleId)) {
                ...
                logger.info(`[Events: MessageReactionAdd] Added role ${roleId} to ${member.user.tag}`);
                
                // Optionally DM user
                try {
                    await member.send(`➕ Peran game **${emoji}** telah ditambahkan ke profil Anda di **${guild.name}**.`);
                } catch (dmErr) {
                    // Ignore DM errors (user may have DMs closed)
                }
            }
        } catch (error) {
            logger.error(`[Events: MessageReactionAdd] Error adding role ${roleId} to user ${user.id}:`, error);
        }
    }
};
